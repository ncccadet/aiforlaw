...
// Helper to list colleges
app.get('/api/admin/get-colleges', async (req, res) => {
  if (req.headers['x-seed-key'] !== 'vfl-seed-2026') return res.status(403).json({ error: 'Forbidden' });
  const seedPool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
  try {
    const { rows } = await seedPool.query('SELECT college_id, name FROM colleges LIMIT 10');
    await seedPool.end();
    res.json({ colleges: rows });
  } catch (err) {
    // Try alternate table name
    try {
      const { rows } = await seedPool.query('SELECT * FROM users LIMIT 1');
      await seedPool.end();
      res.json({ sample_user: rows[0], hint: 'colleges table may not exist - check sample_user for college_id format' });
    } catch (e) {
      await seedPool.end().catch(() => {});
      res.status(500).json({ error: err.message, error2: e.message });
    }
  }
});
...
app.post('/api/admin/seed-users', async (req, res) => {
  ...
  try {
    // Get college_id from the body, or look up an existing one from the DB
    let collegeId = req.body && req.body.college_id;
    if (!collegeId) {
      // Try to get the first college from colleges table
      try {
        const { rows: colleges } = await seedPool.query('SELECT college_id FROM colleges LIMIT 1');
        if (colleges.length > 0) collegeId = colleges[0].college_id;
      } catch (_) {}
    }
    if (!collegeId) {
      // Fall back to copying college_id from an existing user
      const { rows: existingUsers } = await seedPool.query('SELECT college_id FROM users WHERE college_id IS NOT NULL LIMIT 1');
      if (existingUsers.length > 0) collegeId = existingUsers[0].college_id;
    }
    ...
    for (const u of users) {
      ...
      const { rows } = await seedPool.query(
        "INSERT INTO users (email, hashed_password, role, college_id, active_session_version) VALUES ($1, $2, 'student', $3, 0) RETURNING user_id, email, role",
        [u.email, hashed, collegeId]
      );
      results.push({ email: u.email, status: 'created', user_id: rows[0].user_id, college_id: collegeId });
    }
    await seedPool.end();
